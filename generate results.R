...
df$per_households_ElecRes <- as.integer(df$Total..Electricity)/as.integer(df$Total)
df$perc_households_wood <- as.integer(df$Total..Wood)/as.integer(df$Total)

df$perc_households_positiveNB_cool <- df$perc_households_HO*df$perc_positive_NB_HO_cool + df$perc_households_NG*df$perc_positive_NB_NG_cool + 
...
df$perc_households_positivePrivate_cool <- df$perc_households_HO*df$perc_positive_private_HO_cool + df$perc_households_NG*df$perc_positive_private_NG_cool + 
...
df$perc_households_positiveNB_nocool <- df$perc_households_HO*df$perc_positive_NB_HO_nocool + df$perc_households_NG*df$perc_positive_NB_NG_nocool + 
...
df$perc_households_positivePrivate_nocool <- df$perc_households_HO*df$perc_positive_private_HO_nocool + df$perc_households_NG*df$perc_positive_private_NG_nocool + 
...
select(df, contains("perc_"), contains("mean_"), NAME, FIPS)
